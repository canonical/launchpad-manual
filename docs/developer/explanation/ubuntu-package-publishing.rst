...
Please note that this document is not describing the policies and
review processes involved in accepting package changes
or the migrations between suites, but rather the technical steps
...
and the chroot to build it in. The VM then downloads the chroot, unpacks it,
and uses ``sbuild`` to build the package.
...
changes file and built artifacts (mostly debs, ddebs, and the buildinfo file)
...
various checks like ensuring that the architecture of the built debs
...
the archive or due to the package being a kernel package, which are
