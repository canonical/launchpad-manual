...
.. code-block:: python
  ...
  # After that we can clean the builder.
  proxy.clean()

Proxy setup
-----------

Most of the time in our staging environments we will need to set up the proxy for our builders, with the word ``proxy`` we are referring here to both builder proxy and fetch service.
...
The ``_get_value_from_config`` function is a mock function here to indicate that we should retrieve these values from the ``launchpad-buildd-manager`` configuration.
